function [ ok ] = double_2d_is_state_free( state, state_limits, obstacles, radius, time_range,dynamic_obstacles,base_time)
...
for ii=1:n_obs
    ...
    if (s(1)>obs(1) && s(1)<obs(1)+obs(3) && s(2)>obs(2) && s(2)<obs(2)+obs(4)) || ...
       (s(5)>obs(1) && s(5)<obs(1)+obs(3) && s(6)>obs(2) && s(6)<obs(2)+obs(4))
        coll = true;
        return;
    end
    
    closest1 = min(max(s(1:2),obs(1:2)),obs(1:2)+obs(3:4));
    closest2 = min(max(s(5:6),obs(1:2)),obs(1:2)+obs(3:4));
    d1 = s(1:2)-closest1;
    d2 = s(5:6)-closest2;

...
if(true)
    %% Sequential
    load no_obstacle_2d/sequential/Drone1Traj.mat
    d1 = full_path;
    load no_obstacle_2d/sequential/Drone2Traj.mat
    d2 = full_path;
    ...
    plot(d1(:,2),d1(:,3),'LineWidth',3);
    hold on;
    ax = gca
    ax.ColorOrderIndex = 1;
    plot(d2(:,2),d2(:,3),'--','LineWidth',3);
    hold on
    xlim([0,1])
    ylim([0,1])
    view(2)
    %% simultaneous
    if(false)
        load no_obstacle_2d\simultaneous\traj.mat
        d1 = full_path(:,1:5);
        d2 = full_path(:,[1,6:9]);
        ...
        % hold on;
        % patch([d3(:,2);nan],[d3(:,3);nan],[d3(:,4);nan],[d3(:,1)/max(d3(:,1));nan],'EdgeColor','interp','FaceColor','none','LineWidth',5);
        plot(d1(:,2),d1(:,3),'b','LineWidth',3);
        hold on;
        ax = gca
        ax.ColorOrderIndex = 2;
        plot(d2(:,2),d2(:,3),'b--','LineWidth',3);
        hold on;
        xlim([0,1])
        ylim([0,1])
        view(2)
        ...
        saveas(gcf,"no_obstacle_2d/composite.png")
end

function [ ok ] = is_state_free( state, state_limits, obstacles, radius, time_range,dynamic_obstacles,base_time)
...
for ii=1:n_obs
    ...
    if s(7)>obs(1) && s(7)<obs(1)+obs(4) && s(8)>obs(2) && s(8)<obs(2)+obs(5) && s(9)>obs(3) && s(9)<obs(3)+obs(6) 
        coll = true;
        return;
    end
    ...
    closest = min(max(s(7:9),obs(1:3)),obs(1:3)+obs(4:6));
    d = s(7:9)-closest;
    if sum(d.^2) < radius^2
        coll = true;
        return;
...
if(sum((s(1:3)-s(7:9)).^2) <(radius*2)^2)
    coll = true;
    return;
end
